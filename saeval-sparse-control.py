...
for node_names, node_label, attribute in zip(all_nodes, all_nodes_labels, attributes):
    ...
    for idx in tqdm(range(512)):
        ...
        new_attr, clean_logits, patched_logits, corr_logits = ioi_circuit.run_with_patch(
            example, node_names=node_names, attribute=attribute, method="corr", verbose=False
        )
        ...
        scores_df["patch_corr_score"].append(
            logits_score(clean_logits, patched_logits, corr_logits, " " + io, " " + new_attr)
        )
        ...
        _, clean_logits, patched_logits, _ = ioi_circuit.run_with_patch(
            example, node_names=node_names, attribute=attribute, method="feature", patches=patches, verbose=False
        )

        scores_df["patch_supervised_ld"].append(logits_diff(patched_logits, " " + io, " " + new_attr).item())
        scores_df["patch_supervised_score"].append(
            logits_score(clean_logits, patched_logits, corr_logits, " " + io, " " + new_attr)
        )

        ### ALL SAE FEATURES
        _, clean_logits, patched_logits, corr_logits = ioi_circuit.run_with_patch(
            example, node_names=node_names, attribute=attribute, method="sae-all", verbose=False, new_attr=new_attr
        )

        scores_df["patch_all_sae_ld"].append(logits_diff(patched_logits, " " + io, " " + new_attr).item())
        scores_df["patch_all_sae_score"].append(
            logits_score(clean_logits, patched_logits, corr_logits, " " + io, " " + new_attr)
        )
        ...
        _, clean_logits, patched_logits, corr_logits = ioi_circuit.run_with_patch(
            example, node_names=node_names, attribute=attribute, method="sae-fs", verbose=False, new_attr=new_attr
        )

        scores_df["patch_sae_fs_ld"].append(logits_diff(patched_logits, " " + io, " " + new_attr).item())
        scores_df["patch_sae_fs_score"].append(
            logits_score(clean_logits, patched_logits, corr_logits, " " + io, " " + new_attr)
        )
